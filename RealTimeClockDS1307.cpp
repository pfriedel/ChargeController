...
RealTimeClockDS1307::RealTimeClockDS1307()
{
  ...
  //TinyWireM.begin() has not been called; readClock() will hang.
  //Fortunately, it seems that you can call TinyWireM.begin()
...
void RealTimeClockDS1307::readClock()
{
  ...
  TinyWireM.beginTransmission(DS1307_I2C_ADDRESS);
  TinyWireM.send((uint8_t) 0x00);
  TinyWireM.endTransmission();
 
  TinyWireM.requestFrom(DS1307_I2C_ADDRESS, 8);
  _reg0_sec = TinyWireM.receive();
  _reg1_min = TinyWireM.receive();
  _reg2_hour = TinyWireM.receive();
  _reg3_day = TinyWireM.receive();
  _reg4_date = TinyWireM.receive();
  _reg5_month = TinyWireM.receive();
  _reg6_year = TinyWireM.receive();
  _reg7_sqw = TinyWireM.receive();
}
...
void RealTimeClockDS1307::setClock()
{
  ...
  TinyWireM.send(_reg1_min);
  TinyWireM.send(_reg2_hour);
  TinyWireM.send(_reg3_day);
  TinyWireM.send(_reg4_date);
  TinyWireM.send(_reg5_month);
  TinyWireM.send(_reg6_year);
  TinyWireM.endTransmission();
  //now, we'll send the seconds; we didn't have to keep
  //track of whether the clock was already running, because
  //_reg0_sec already knows what we want it to be. This
  //will restart the clock as it sends the new seconds value.
...
byte RealTimeClockDS1307::readData(byte regNo)
{
  ...
  TinyWireM.send(regNo);
  TinyWireM.endTransmission();
  TinyWireM.requestFrom(DS1307_I2C_ADDRESS, 1);
...
void RealTimeClockDS1307::readData(byte regNo, void * dest, int length)
{
  ...
  TinyWireM.send(regNo);
  TinyWireM.endTransmission();
  TinyWireM.requestFrom(DS1307_I2C_ADDRESS, length);
  for(int i=0; i<length; i++) {
    *p=TinyWireM.receive();
    p++;
  }
